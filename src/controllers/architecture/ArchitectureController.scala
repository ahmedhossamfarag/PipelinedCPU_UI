...
class ArchitectureController(val arch: Architecture){
  ...
  def fetch: IF_ID = {
    ...
    arch.pc = if (branchControl.will) arch.ex_mem.nextPc else arch.pc + 4
  ...
  def decode: ID_EX = {
    ...
    id_ex.data1 = registersControl.get(arch.if_id.inst.src_1).getOrElse(0)
    id_ex.data2 = registersControl.get(arch.if_id.inst.src_2).getOrElse(0)
